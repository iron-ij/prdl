...
def define_argparser():
    ...
    p.add_argument('--files', required=True, type=str, help="input corpus parent directory path")
    ...
    # CBPE init args
    p.add_argument('--merges', type=Optional[Union[str, Dict[Tuple[int, int], Tuple[int, int]]]], default=None)
    ...
    p.add_argument('--strip_accents', type=Optional[bool], default=None) # Must be False if lowercase is False
    p.add_argument('--wordpieces_prefix', type=str, default="##")

    # Wordpiece train args
    p.add_argument('--word_piece_special_tokens', default=[
            "[PAD]",
            "[UNK]",
            "[CLS]",
            "[SEP]",
...
if __name__ == "__main__":
    ...
    if config.sample_rate:
        file_list = [str(files.absolute()) for files in Path(config.files).glob('*.txt')]
        procs=[]

        for i in file_list:
            proc = multiprocessing.Process(target=sampler.execute_script, args=(i, config.sample_rate, ))
            procs.append(proc)
            proc.start()
        ...
        source_list = [input_file.replace('corpus', 'sample') for input_file in file_list]
    else:
        source_list = [str(files.absolute()) for files in Path(config.files).glob('*.txt')]
